...
time_sql_cmd() {
    ...
    # Calculate duration in milliseconds using awk for portability
    duration_ms=$(awk "BEGIN {printf \"%.3f\", (${end_ns} - ${start_ns}) / 1000000}")

    if [[ -n "${expected_rows}" ]]; then
        row_count="${expected_rows}"
    else
        # Extract row count from output (works for SELECT COUNT queries)
...
record_result() {
    ...
    # Calculate keys per second (avoid division by zero)
    local keys_per_sec
